...
def events(db: Session, shocks_only: bool) -> list[IntelligenceEventOut]:
    ...
    if shocks_only:
        query = query.where(IntelligenceEvent.event_type.in_(["SHOCK", "AIRFARE_SHOCK"]))
    return [
        IntelligenceEventOut(
            id=e.id,
            route=e.route.code if e.route else None,
            event_type=e.event_type,
            anomaly_score=e.anomaly_score,
            pressure_score=e.pressure_score,
            shock_status=e.shock_status,
            explanation=e.explanation,
            affected_sources=e.affected_sources,
